...
The CASMSocial project team recommends this sample fixture for public release
under MIT. Final public-release approval remains with the package reviewer, so
its current manifest status is
`recommended_for_public_release_pending_final_review`. Until that decision is
recorded, the fixture is retained only in the private release candidate and is
not included in public distribution artifacts. This is an internal release
gate, not an additional restriction on the MIT license that will apply if the
fixture is approved and released.
