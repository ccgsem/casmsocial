...
def create_mvp_tables(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    conn.execute("""
        CREATE OR REPLACE TABLE rti_synth_pop_v2_dmv_100.places (
            sp_id BIGINT,
            rank INTEGER,
            place_type VARCHAR,
            place_name VARCHAR,
        ...
        """)
    conn.execute("""
        CREATE OR REPLACE TABLE rti_synth_pop_v2_dmv_100.persons (
            sp_id BIGINT,
            sp_hh_id BIGINT,
            sp_work_id BIGINT,
            sp_school_id BIGINT
        ...
        """)
    conn.execute("""
        CREATE OR REPLACE TABLE rti_synth_pop_v2_dmv_100.hh (
            sp_id BIGINT,
            hh_size INTEGER,
            hh_income DOUBLE,
            hh_type VARCHAR,
        ...
        """)
    conn.execute("""
        CREATE OR REPLACE TABLE rti_synth_pop_v2_dmv_100.activities (
        ...
        """)
    conn.execute("""
        CREATE OR REPLACE TABLE rti_synth_pop_v2_dmv_100.road_nodes (
            node_id BIGINT,
            x DOUBLE,
            y DOUBLE
        )
        """)
    conn.execute("""
        CREATE OR REPLACE TABLE rti_synth_pop_v2_dmv_100.road_edges (
            edge_id BIGINT,
            from_node_id BIGINT,
            to_node_id BIGINT,
            length_m DOUBLE,
        ...
        """)
    conn.execute("""
        CREATE OR REPLACE TABLE rti_synth_pop_v2_dmv_100.place_road_snap (
            place_id BIGINT,
            road_node_id BIGINT
        )
        """)
    conn.execute("""
        ...
        """)
    conn.execute("""
        INSERT INTO rti_synth_pop_v2_dmv_100.places VALUES
            (100, 0, 'Household', 'home-a', 38.90, -77.04),
            (200, 0, 'Household', 'home-b', 38.91, -77.05),
            (300, 0, 'Workplace', 'work', 38.92, -77.06)
        """)
    conn.execute("""
        INSERT INTO rti_synth_pop_v2_dmv_100.persons VALUES
            (1, 100, 300, NULL),
            (2, 200, 300, NULL)
        """)
    conn.execute("""
        INSERT INTO rti_synth_pop_v2_dmv_100.hh VALUES
            (100, 1, 75000.0, 'single', 'unknown', 35),
            (200, 1, 82000.0, 'single', 'unknown', 41)
        """)
    conn.execute("""
        INSERT INTO rti_synth_pop_v2_dmv_100.activities VALUES
            (1, 0, 0, 0, 480, 100),
            (1, 1, 1, 540, 1020, 300),
            (1, 0, 2, 1080, 1439, 100),
            (2, 0, 0, 0, 480, 200),
        ...
        """)
    conn.execute("""
        INSERT INTO rti_synth_pop_v2_dmv_100.road_nodes VALUES
            (1, -77.04, 38.90),
            (2, -77.05, 38.91),
            (3, -77.06, 38.92)
        """)
    conn.execute("""
        INSERT INTO rti_synth_pop_v2_dmv_100.road_edges VALUES
        ...
        """)
    conn.execute("""
        INSERT INTO rti_synth_pop_v2_dmv_100.place_road_snap VALUES
            (100, 1),
            (200, 2),
            (300, 3)
        """)
