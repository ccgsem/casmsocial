...
class Human(core.Agent):
    ...
    def walk(self, grid):
        ...
        # x and y dimensions
        xy_dirs = random.default_rng.choice(Human.OFFSETS, size=2)
...
def restore_human(human_data: Tuple):
    """
    Args:
    ...
    uid = human_data[0]
    pt_array = human_data[2]
    pt = dpt(pt_array[0], pt_array[1], 0)

    if uid in human_cache:
        human = human_cache[uid]
    else:
        human = Human(uid[0], uid[2], pt)
        human_cache[uid] = human

    human.meet_count = human_data[1]
    human.pt = pt
    return human
