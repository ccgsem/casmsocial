...
def test_committed_fixture_records_team_recommendation_and_final_review_gate() -> None:
    manifest_path = Path("testdata/wake_county_heat_1000_households/manifest.yaml")
    manifest = yaml.safe_load(manifest_path.read_text(encoding="utf-8"))
    classification = manifest["data_classification"]

    assert classification["status"] == "recommended_for_public_release_pending_final_review"
    assert classification["proposed_license"] == "MIT"
    assert classification["approval"]["project_team_decision"] == "recommended_for_public_release"
    assert classification["approval"]["final_reviewer_decision"] == "pending"
    assert classification["approval"]["evidence_uri"] == ""
    assert "does not add restrictions" in classification["release_process"]["license_effect"]
