...
`recommended_for_public_release_pending_final_review`.

Until the reviewer records the decision, date, and evidence URI, the fixture is
retained only in the private release candidate and is not included in public
distribution artifacts. This records an internal release gate; it does not add
restrictions to the MIT license that will apply if the fixture is approved and
released.
